...
import { entrepreneurs, products, categories, banners, news, stats, contacts, communes, reviews } from "@db/schema";
import { eq, and, like, or, desc, asc, sql, count, gte, avg } from "drizzle-orm";
...
// Reviews Queries
// ==========================================
export async function findReviewsByEntrepreneurId(entrepreneurId: number, status?: "pending" | "approved" | "rejected") {
  const db = getDb();
  const conditions = [eq(reviews.entrepreneurId, entrepreneurId)];
...
export async function findAllReviews(filters?: {
  ...
  const db = getDb();
  const conditions = [];
  if (filters?.entrepreneurId) conditions.push(eq(reviews.entrepreneurId, filters.entrepreneurId));
...
export async function createReview(data: typeof reviews.$inferInsert) {
  const db = getDb();
  const [{ id }] = await db.insert(reviews).values(data).$returningId();
...
export async function getAverageRating(entrepreneurId: number) {
  const result = await getDb()
    ...
    .where(
      and(
        eq(reviews.entrepreneurId, entrepreneurId),
        eq(reviews.status, "approved")
      )
    );

  return {
    average: result[0]?.average ? parseFloat(Number(result[0].average).toFixed(1)) : 0,
